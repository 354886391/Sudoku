 ...
 */
    public static checkIsLowPhone(): Boolean {
        ...
        if (window.wx) {
            ...
            if (nowBenchmarkLevel < 20) { // 22的具体参数可参考微信官方
                return true;
            } else {
                return false;
            }
    ...
    * @param duration 震屏时间
    */
    public static shakeEffect(node: Node, duration: number) {
        let shakeTween = tween(node)
            .to(0.02, { position: v3(0.00, 10.00) })
            .to(0.02, { position: v3(-7.07, -7.07) })
            .to(0.02, { position: v3(10.00, -0.00) })
            .to(0.02, { position: v3(-7.07, 7.07) })
            .to(0.02, { position: v3(-0.00, -10.00) })
            .to(0.02, { position: v3(7.07, 7.07) })
            .to(0.02, { position: v3(-10.00, 0.00) })
    ...
    public static fadePanel(root: Node, duration: number, isOpen: boolean = false) {
        if (root instanceof Node) {
            root.walk((child) => {
                let render = child.getComponent(UIRenderer);
                if (render && render.node.isValid) {
                    let alpha = render.color.a;
                    let sAlpha: number = isOpen ? alpha * 0.8 : alpha;
                    let tAlpha: number = isOpen ? alpha : alpha * 0.8;
                    let oAlpha: IAlpha = { target: sAlpha };
                    tween(oAlpha)
                        .to(duration, { target: tAlpha }, {
                            easing: "linear",
                            onUpdate(data: IAlpha, ratio) {
                                let tColor = render.color;
                                let cAlpha = Math.round(data.target);
                                render.color = new Color(tColor.r, tColor.g, tColor.b, cAlpha);
                            }
                        })
                        .start();
                }
