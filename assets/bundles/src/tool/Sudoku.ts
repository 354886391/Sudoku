...
export class Sudoku {
    ...
    initialize() {
        ...
        SQUARES = this._cross(ROWS, COLS);
        UNITS = this._get_all_units(ROWS, COLS);
        SQUARE_UNITS_MAP = this._get_square_units_map(SQUARES, UNITS);
        SQUARE_PEERS_MAP = this._get_square_peers_map(SQUARES,
            SQUARE_UNITS_MAP);
    }
    ...
    generate(difficulty, unique) {
        ...
        */
        ...
        difficulty = this._force_range(difficulty, NR_SQUARES + 1,
            MIN_GIVENS);
        ...
        */
        ...
        if (!reverse) {
            for (var vi in min_candidates) {
                ...
                // TODO: Implement a non-rediculous deep copy  
    ...
    validate_board(board) {
        ...
        */
        ...
        for (var i in board) {
            if (!this._in(board[i], DIGITS) && board[i] !== BLANK_CHAR) {
